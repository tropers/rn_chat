...
class P2PTest(unittest.TestCase):
    ...
    def __check_container_stdout_does_not_contain(self, containers: Dict[str, P2PContainer],
                                                  container_name: str, regex: str):
        if re.search(regex, containers[container_name].get_stdout_utf8()):
            logger.info('[FAIL] Regex "%s" found in output of container %s!', regex, container_name)
            self.fail()
